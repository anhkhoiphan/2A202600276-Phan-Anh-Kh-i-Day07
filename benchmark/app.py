...
# ── global state (avoid gr.State deepcopy issue with non-picklable objects) ───
_STORES:  dict = {}
_QUERIES: list = []
...
def load_stores() -> tuple[dict, str]:
    ...
    if not stores:
        ...
        source = next(
            (json.loads((EMBEDDINGS_DIR / f"{s}.json").read_text(encoding="utf-8"))["source"]
             for s in STRATEGIES if (EMBEDDINGS_DIR / f"{s}.json").exists()),
            "unknown"
        )
        embedder_name = next(
            (json.loads((EMBEDDINGS_DIR / f"{s}.json").read_text(encoding="utf-8"))["embedder"]
             for s in STRATEGIES if (EMBEDDINGS_DIR / f"{s}.json").exists()),
            "unknown"
        )
        status = (
...
def build_ui() -> gr.Blocks:
    ...
    # Fix Gradio 6.0: theme moved to launch(), not Blocks()
    with gr.Blocks(title="RAG Benchmark Demo") as demo:
        ...
        # Wiring — no gr.State needed anymore
...
if __name__ == "__main__":
    ui = build_ui()
    ui.launch(server_name="127.0.0.1", server_port=7860, share=False, theme=gr.themes.Soft())